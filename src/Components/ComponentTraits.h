...
struct ComponentTraits<Camera_C>
{
    ...
    static void upload(ResourceManager& r, Camera_C& c, uint32_t frameIndex)
    {
        ...
        {
            auto resourceGuid = r.createBufferFrameResource(
                ResourceManager::AlignUp(sizeof(Camera_C::Data), 256), D3D12_RESOURCE_STATE_COMMON,
                D3D12_HEAP_TYPE_DEFAULT, "camera");
            c._buffer_ID = r.createFrameCBV(resourceGuid, std::nullopt, 0, sizeof(Camera_C::Data));
            c._dirty.markAll(true);
        }

        if (!c._dirty.isDirty(frameIndex))
            return;

        r.requestUpload(c._buffer_ID, &c._data, sizeof(Camera_C::Data), 256);
