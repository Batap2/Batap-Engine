...
private:
    ...
    void Initialize(ID3D12Device* device, ID3D12GraphicsCommandList* commandList,
                    ...
                    const std::vector<T>& data, bool allowUAV = false)
    {
        ...
        device->CreateShaderResourceView(resource.Get(), &srvDesc, gpuSrvDescriptorHandle);
    }
...
public:
    template <typename T>
    void CreateOrUpdateStructuredBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* commandList,
        ID3D12DescriptorHeap* descriptorHeap, unsigned int& descriptorIndex,
        const std::vector<T>& data, bool allowUAV = false)
    {
        UINT bufferSize = sizeof(T) * static_cast<UINT>(data.size());

        // If the buffer already exists and is the correct size, just update it
        ...
        Initialize(device, commandList, descriptorHeap, descriptorIndex, data, allowUAV);
    }
};