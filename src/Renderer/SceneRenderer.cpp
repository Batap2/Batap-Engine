...
namespace rayvox
{
    // SceneRenderer::SceneRenderer(ResourceManager* resourceManager) : _resourceManager(resourceManager){}

    // void SceneRenderer::recordCommandsOpaqueMesh(ID3D12GraphicsCommandList* cmdList, uint32_t frameIndex){
    ...
    void SceneRenderer::initRenderPasses(){
        ...
        .addRecordStep(
            [this](ID3D12GraphicsCommandList* cmdList, uint32_t frameIndex)
            {
                ...
                cmdList->Dispatch(r->_threadGroupCountX, r->_threadGroupCountY, r->_threadGroupCountZ);
            });
    }
}  // namespace rayvox